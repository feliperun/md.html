...
- Rejects `@import` in any form, `@namespace`, and unknown at-rules
  (`E-MDHSEC-008`) — the build fails, nothing is silently removed.
- Rejects network `url()` (`E-MDHSEC-009`); allows only `data:`,
  embedded-asset-map references, and runtime `blob:` for asset-map lazy
  images.
...
- Exact version pinning after `cargo audit`, RUSTSEC verification, the Sentrux
  ...
  the release budget are mandatory before the crate lands. ADR 0019 raised
  that budget from 600 KiB to 3 MiB after measuring `lightningcss` at
  1,882,704 bytes with both parser crates, so the 600 KiB fallback trigger
  recorded here is unattainable and the `cssparser` + `selectors` fallback
  stays dormant unless a future ADR lowers the budget again.
...
- The policy splits visual customization (kept — PRD Constraint 3) from
  execution, external communication, and host interference (rejected).
  Malformed CSS fails the build (`E-MDHSEC-007`); disallowed constructs fail
  the build with their `E-MDHSEC-008`..`-010` code — reject, fail-closed,
  symmetric with the HTML guard — and source bytes are untouched, preserving
  byte-perfect extraction (PRD Constraint 2, "reject rather than silently
  mutate"). Deterministic re-serialization applies only to CSS that already
  passed the policy, as a stable output format.
